...
function kama_get_most_viewed($args=''){
	...
	foreach( $results as $pst ){
     ...
     $ffraviev = get_post_meta ($post->ID,'views',true);
        $ffracom = get_comments_number();
        $cat = get_the_category(); 
        $ffranam = $cat[0]->cat_name;
        $ffraname = get_the_title();
		$a1 = "<a href='". get_permalink($pst->ID) ."' title='{$pst->views} просмотров: $Title' class='cols element col-1'>";
		$a3 = "<div class='img-container' style='overflow:hidden;'><img src='$thumb_url[0]' style='width:100%;'><div class='views-comment'>
							<span class='views'>{$pst->views}</span>
							<span class='comment'>$ffracom</span>
						</div></div>
                        <div class='name'>$ffranam</div>
